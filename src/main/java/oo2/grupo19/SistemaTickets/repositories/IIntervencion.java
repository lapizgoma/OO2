...
public interface IIntervencion extends JpaRepository<Intervencion,Long> {
	...
	List<Intervencion> traer(@Param("fecha") LocalDateTime fecha, @Param("usuario") Usuario usuario);

    @Query("select m from Intervencion m where m.realizadoPor.id = :idEmpleado")
	List<Intervencion> traerIntervencionPorEmpleado(@Param("idCEmpleado") Long idEmpleado);
    
    @Query("select m.realizadoPor from Intervencion m where m.realizadoPor.id = :idEmpleado ")
	Empleado traerEmpleadoDesdeIntervencion(@Param("idEmpleado") Long idCliente);
