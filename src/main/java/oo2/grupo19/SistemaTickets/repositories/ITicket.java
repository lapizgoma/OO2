...
public interface ITicket extends JpaRepository<Ticket,Long> {
    ...
    @Query("select t from Ticket t where t.estado.estado = :estado")
    List<Ticket> traerPorEstado(@Param("estado") String estado);

    @Query("select t from Ticket t where t.prioridad.prioridad = :prioridad")
    List<Ticket> traerPorPrioridad(@Param("prioridad") String prioridad);
